...
function setLinkActive(){
	...
	$('#sell_nav').addClass('menu-item-active');
}
	...
	$.ajax({
		...
		success: function(response){      
			for( var i = 0; i<response.data.length; i++){
				...
				populateStockInHand(response.data[i]['product']['productId'],'[0][edit_sale_product_stock]');
...
function calculateSaleTotalForEdit(value,param,flag){
	var totalElements = $("input[name='edit_sale_total_elements']").val();
	var sell_price = 0;
	var total = 0;
	if(totalElements == 0){
		if(!flag){
			sell_price = $('input[name="['+ param.substring(1,2) +'][edit_product_selling_price]"').val();
			total = value * sell_price;	
		}else{
			var sell_quantity = $('input[name="['+ param.substring(1,2) +'][edit_product_quantity]"').val();
		...
		for(var i = 0;i<=totalElements;i++){
			...
			sell_price = $('input[name="['+ i +'][edit_product_selling_price]"').val();
...
function populateStockInHand(id,name){
	if('Select' != id){
		$.ajax({
			...
			dataType: 'json',
			success: function(response){      
				$('input[name="['+ name.substring(1,2) +'][edit_sale_product_stock]"]').val(response.stockQuantity);
			}
		});
	}else{
		$('input[name="['+ name.substring(1,2) +'][edit_sale_product_stock]"]').val('');
	}
}
...
function checkStock(){
	var invalid = false;
	$("input[name*='edit_sale_product_stock']").each(function(){
		if($(this).val() == 0){
			invalid = true;
		}
	});
				...
				if (status == 'Valid') {
					...
					if (invalid) {
						...
						if(checkStock()){
							swal.fire(
									{
										text : "Cannot sell products that are not in stock. kindly order products that are out of stock to sell them.",
										icon : "error",
										buttonsStyling : false,
										confirmButtonText : "Ok, got it!",
										customClass : {
