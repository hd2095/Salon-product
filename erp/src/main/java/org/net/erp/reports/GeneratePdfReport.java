...
public class GeneratePdfReport {
	...
	public static ByteArrayInputStream invoicePdf(HashMap<String,String> pdfContents,List<SaleDetails> allSaleDetails) {
		...
		try { 
			...
			System.out.println("Exception in invoicePdf :: "+e.getMessage());
		}finally {
			try {
				...
				System.out.println("Exception in closing invoicePdf :: "+e.getMessage());
			}
		}
		return new ByteArrayInputStream(out.toByteArray()); 
	}

	public static ByteArrayInputStream invoiceAppointmentPdf(HashMap<String,String> pdfContents,List<AppointmentDetails> appointmentDetails) {
		ByteArrayOutputStream out = new ByteArrayOutputStream();
		PdfWriter writer = new PdfWriter(out); 
		PdfDocument pdfDoc = new PdfDocument(writer); 
		Document document = new Document(pdfDoc); 
		try {
			...
			System.out.println("Exception in invoiceAppointmentPdf :: "+e.getMessage());
		}finally {
			try {
				if(null != pdfDoc && !pdfDoc.isClosed()) {
					pdfDoc.close();
